...
Общие конвенции по параметрам. Имя флага оффлайн-режима — `noFetch` (алиас `no_fetch` тоже принимается). Методы, принимающие `version`, понимают значение `latest` как «резолвить актуальный последний релиз AMXX, игнорируя манифест»; любое другое значение должно быть конкретной версией `major.minor.build` — иначе ошибка `-32602`.
...
| `noFetch` | boolean | Не ходить в сеть, только кэш |
...
### Manifest-less вызовы (без amxbuild.yml)

Методы `include.resolve`, `amxmodx.includes.list`, `amxmodx.include.get` и `compiler.info` — информационные и работают **без манифеста вообще**. Сценарий: клиент (например, расширение редактора) стоит над отдельным или несохранённым `.sma` вне какого-либо проекта AMXX и хочет получить stdlib-инклюды последней версии AMXX — никакого `amxbuild.yml` рядом нет.

- `manifest` и `version` можно не передавать вовсе: версия берётся как latest (сеть при первом обращении, дальше — кэш версии ~1 час). Если в cwd всё же есть манифест, его `amxmodx.version` учитывается как значение по умолчанию.
- `version: "latest"` явно резолвит актуальный последний релиз, **игнорируя** манифест, автоопределённый из cwd.
- `noFetch: true` — полный оффлайн: никакого network-обращения — ни lookup-версии, ни скачивания компилятора. Если запрошенной/latest версии в кэше нет, но какой-то компилятор кэширован — метод отвечает по новейшему кэшированному компилятору и помечает ответ флагом `degraded: true`. Если кэш компиляторов пуст — возвращается «пустое» graceful-состояние (не ошибка): `version: null` и пути/файлы `null`/`[]`.
- `include.list` (список `.inc` зависимостей манифеста) — **не** manifest-less: он требует манифест и работает по его `deps`.
...
| `version` | string | Версия AMXX (по умолчанию: из манифеста, иначе latest). Значение `latest` резолвит актуальный последний релиз, игнорируя версию из манифеста |
| `noFetch` | boolean | Не ходить в сеть: ни за версией, ни за скачиванием компилятора. Ответ отражает только кэш; подробнее см. «Manifest-less вызовы» |
| `pattern` | string | Glob-фильтр (по умолчанию `*.inc`) |

Ответ: `{ "version", "includeDir", "pattern", "count", "files": ["amxmodx.inc", ...] }`. С `noFetch: true` при пустом кэше компиляторов — graceful-состояние (не ошибка): `{ "version": null, "includeDir": null, "pattern": "*.inc", "count": 0, "files": [] }`; если запрошенной/latest версии в кэше нет, но другой компилятор кэширован — список строится по новейшему кэшированному, а ответ помечается `degraded: true`.
...
| `version` | string | Переопределить версию AMXX. Значение `latest` резолвит актуальный последний релиз, игнорируя манифест |
...
| `manifest` | string | Для определения версии из `amxmodx.version` |
| `version` | string | Версия AMXX (по умолчанию: из манифеста, иначе latest). Значение `latest` резолвит актуальный последний релиз, игнорируя версию из манифеста |
| `noFetch` | boolean | Не ходить в сеть: ни за версией, ни за скачиванием компилятора. Если запрошенная версия не в кэше — ответ отражает только кэш; если latest неизвестен (кэш версии протух/пуст) — возвращается новейший кэшированный компилятор с `degraded: true`, а при пустом кэше — graceful-состояние `version: null` (не ошибка) |
...
Без `noFetch` компилятор будет скачан при первом обращении (как в обычной сборке), а версия без явного `version` резолвится в сеть. С `noFetch: true` сеть не используется вовсе:

- если запрошенная (или latest) версия уже кэширована — ответ как в примере выше;
- если её в кэше нет, но кэширован какой-то другой компилятор — ответ строится по новейшему кэшированному компилятору и получает флаг `degraded: true` (поле отсутствует, когда ответ точный);
- если кэш компиляторов пуст — graceful-состояние, **не ошибка**: `{ "version": null, "platform": "linux", "compilerPath": null, "includeDir": null, "cached": false }`.

Точный ли ответ или деградированный, клиент определяет по наличию `degraded`; `version: null` означает «в кэше нет ни одного компилятора».
...
> **WSL и пути `/mnt/*`.** Локальные `.sma` компилируются напрямую из папки проекта,
> а `amxxpc` (32-битный Linux-бинарник) не читает файлы на WSL-монтированиях `/mnt/*`
> (DrvFs/9p) — сборка падает с `fatal error 100: cannot read from file`. Плагины из
> `repos` компилируются из нативного кэша и этой проблемы не имеют. `--build-dir`/
> `buildDir` не помогают: они меняют только staging-папку, но не путь чтения исходников.
> Запускайте сборку из нативной Linux-папки (`~`), либо на Windows, либо в CI.
...
> **WSL и пути `/mnt/*`.** `amxxpc` — 32-битный Linux-бинарник; он **не читает** файлы
> на WSL DrvFs/9p-монтированиях (`/mnt/c`, `/mnt/j`, …). Если `sma_file` или любая
> `include_dirs` лежит под `/mnt/*`, компиляция падает с
> `fatal error 100: cannot read from file`, а папка-инклюд может уронить компилятор
> (`std::bad_alloc` / SIGABRT) — при том что файл существует и читается из Node.
> Запись результата на `/mnt/*`, наоборот, работает. Это ограничение окружения, а не
> кода. Альтернатива — компилировать из нативной Linux-файловой системы: скопируйте
> `.sma` и соседнюю `include/` в `~/` (кэш `~/.cache/amxx-builder` общий, повторно
> ничего не скачивается) и передайте путь оттуда, либо выполняйте сборку на Windows.
...
- **Сеть.** Запросы, которые тянут репозитории/релизы/компилятор (`repos.*`, `releases.list`, `include.list`, `deps.tree`, `build.start`, ...), ходят в GitHub API и могут быть медленными или упереться в rate-limit (403/429 — см. `error.data.status`). `noFetch: true` ограничивает работу кэшем там, где это применимо.
