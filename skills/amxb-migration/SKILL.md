...
4. **Если после разбора CI и скриптов сборки источник не находится — НЕ копаем
   ...
   лежит. Скорее всего зависимость приватная, и пользователь даст токен
   (обычно через `.env` рядом с манифестом — amxb читает его сам). Вопрос
   задаём по каждому неопознанному include, а не по одному самому очевидному.
...
`amxx-dep-resolver` (`list_releases`, `get_dep_tree`), **но только если они
реально доступны в текущей сессии** — MCP подхватывается после перезапуска
opencode (см. Шаг 9). Если инструментов нет — используем CLI `amxb releases` /
`amxb deps-tree`: они дают ту же информацию и работают без перезапуска.
Ручные запросы к GitHub API/вебу для выбора версии не нужны и не делаются —
см. «Формат deps» ниже. Чужие/соседние репозитории и их содержимое при этом
не открываем.
