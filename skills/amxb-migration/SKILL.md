...
7. Помнить особенности amxb:
   ...
   - `.sma`-файлы **и копируются** в пакет (как любые файлы), **и
     компилируются**; если исходники не должны попадать в архив, их можно
     исключить: для репо через `exclude:`/`exclude_files:` в правиле
     репозитория, для локальной `amxmodx/` через `amxmodx.exclude_files`
     (glob-паттерны относительно `amxmodx.dir`);
