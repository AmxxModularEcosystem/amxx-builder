...
Автоматически — командой `amxb init --opencode`: она создаёт `.opencode/opencode.json`
с этим MCP-конфигом и файл `.opencode/plugin/amxb-skills.js` — тонкий мост, который при
каждом старте opencode через `amxb skills-dir` регистрирует скиллы из текущей установки
amxb (абсолютных путей в конфиге нет). Повторный запуск пропускает существующие файлы
...
`.opencode/plugin/amxb-skills.js` регистрирует их из установки amxb при каждом старте.
